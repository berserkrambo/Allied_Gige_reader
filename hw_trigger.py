...
class AlliedCamera(threading.Thread):
    def __init__(self, camera, exposure, gain, queue):
        super().__init__()
        ...
        self.cam.TriggerSource.set('Line3')
    ...
    def __call__(self,cam, stream, frame):
        ...
        last_frame = last_frame.as_opencv_image()
        self.queue.put_nowait([self.id, last_frame])

        cam.queue_frame(frame)

        print('camera {}, Frame acquired: {}'.format(self.id, frame), flush=True)
...
class AlliedWrapper():
    def __init__(self, exposure, gain):
        ...
        self.queues = [Queue() for i in range(len(camera_list))]
        self.caps = [AlliedCamera(cam, exposure, gain, self.queues[i]) for i, cam in enumerate(camera_list)]
        print("cam setup end")
...
def main ():
    ...
    while True:
        ...
        data = allied_cams.wait_for_frames()
